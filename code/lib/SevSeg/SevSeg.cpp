...
void SevSeg::DisplayString(char* toDisplay, byte DecAposColon)
{
	...
	for(byte digit = 1 ; digit < (numberOfDigits+1) ; digit++)
	{
		...
		{
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<5)) digitalWrite(segmentB, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<6)) digitalWrite(segmentA, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<4)) digitalWrite(segmentC, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<3)) digitalWrite(segmentD, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<2)) digitalWrite(segmentE, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<1)) digitalWrite(segmentF, SegOn);
			if (pgm_read_byte(&characterArray[(int) characterToDisplay]) & (1<<0)) digitalWrite(segmentG, SegOn);
		}
